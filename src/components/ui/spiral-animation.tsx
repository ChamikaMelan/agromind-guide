...
class AnimationController {
  ...
  // dot colour – AgroMind leaf green
  private readonly dotColor = '#7eca5e';
  ...
  public render() {
    ...
    // dark green background instead of pure black
...
class Star {
  ...
  render(p: number, c: AnimationController) {
    ...
    if (dp < 0.3) {
      ...
      sx = c.lerp(bx, spiralPos.x + td * Math.cos(sa), fin);
      sy = c.lerp(by, spiralPos.y + td * Math.sin(sa), fin);
    }
    ...
    let sm = dp < 0.6
      ? 1.0 + dp * 0.2
      : c.lerp(1.2, this.finalScale, (dp - 0.6) / 0.4);
